...
template <class MACHO_T> void BinaryParser::parse(void) {
...
template <class MACHO_T> void BinaryParser::parse_header(void) {
  using header_t         = typename MACHO_T::header;
  this->binary_->header_ = &this->stream_->peek<header_t>(0);
}

template <class MACHO_T> void BinaryParser::parse_load_commands(void) {
  using header_t          = typename MACHO_T::header;
  ...
  for (size_t i = 0; i < nbcmds; ++i) {
    ...
    case LOAD_COMMAND_TYPES::LC_SEGMENT: {
      ...
      load_command          = std::unique_ptr<SegmentCommand>{
          new SegmentCommand{&this->stream_->peek<segment_command_t>(loadcommands_offset)}};
      ...
      const uint8_t *content =
          this->stream_->peek_array<uint8_t>(segment->file_offset(), segment->file_size(), /* check */ false);
      if (content != nullptr) {
        segment->content({content, content + segment->file_size()});
      } else {
    ...
    case LOAD_COMMAND_TYPES::LC_ID_DYLINKER: {
      ...
      std::string name                  = this->stream_->peek_string_at(loadcommands_offset + linker_name_offset);
    ...
    case LOAD_COMMAND_TYPES::LC_UNIXTHREAD: {
      ...
      VLOG(VDEBUG) << "FLAVOR: " << cmd->flavor << std::endl << "COUNT:  " << cmd->count;
      switch (this->binary_->header().cpu_type()) {
      case CPU_TYPES::CPU_TYPE_X86: {
        const uint8_t *pstart = this->stream_->peek_array<uint8_t>(loadcommands_offset + sizeof(thread_command),
                                                                   sizeof(x86_thread_state_t), /* check */ false);
        if (pstart != nullptr) {
      ...
      case CPU_TYPES::CPU_TYPE_X86_64: {
        const uint8_t *pstart = this->stream_->peek_array<uint8_t>(loadcommands_offset + sizeof(thread_command),
                                                                   sizeof(x86_thread_state64_t), /* check */ false);
      ...
      case CPU_TYPES::CPU_TYPE_ARM: {
        const uint8_t *pstart = this->stream_->peek_array<uint8_t>(loadcommands_offset + sizeof(thread_command),
                                                                   sizeof(arm_thread_state_t), /* check */ false);
      ...
      case CPU_TYPES::CPU_TYPE_ARM64: {
        const uint8_t *pstart = this->stream_->peek_array<uint8_t>(loadcommands_offset + sizeof(thread_command),
                                                                   sizeof(arm_thread_state64_t), /* check */ false);
      ...
      default: { LOG(ERROR) << "Unknown architecture"; }
      }
      break;
    }
    ...
    case LOAD_COMMAND_TYPES::LC_BUILD_VERSION: {
      ...
      for (size_t i = 0; i < cmd->ntools; ++i) {
        const uint64_t cmd_offset =
            loadcommands_offset + sizeof(build_version_command) + i * sizeof(build_tool_version);
    ...
    case LOAD_COMMAND_TYPES::LC_CODE_SIGNATURE: {
      ...
      const uint8_t *content =
          this->stream_->peek_array<uint8_t>(sig->data_offset(), sig->data_size(), /* check */ false);
      if (content != nullptr) {
    ...
    case LOAD_COMMAND_TYPES::LC_DATA_IN_CODE: {
      ...
      const data_in_code_entry *entries =
          this->stream_->peek_array<data_in_code_entry>(datacode->data_offset(), nb_entries, /* check */ false);
      if (entries != nullptr) {
    ...
    default: {
      LOG(WARNING) << "Command '" << to_string(static_cast<LOAD_COMMAND_TYPES>(command.cmd)) << "' not parsed";
    ...
    if (load_command != nullptr) {
      const uint8_t *content =
          this->stream_->peek_array<uint8_t>(loadcommands_offset, command.cmdsize, /* check */ false);
      if (content != nullptr) {
        load_command->data({content, content + command.cmdsize});
      }
...
template <class MACHO_T> void BinaryParser::parse_relocations(Section &section) {
  ...
  for (size_t i = 0; i < numberof_relocations; ++i) {
    ...
    if (is_scattered) {
      const scattered_relocation_info *reloc_info =
          &this->stream_->peek<scattered_relocation_info>(current_reloc_offset);
      ...
      if (reloc_info->r_extern == 1 and reloc_info->r_symbolnum != R_ABS) {
        if (reloc_info->r_symbolnum < this->binary_->symbols().size()) {
          ...
          LOG(WARNING) << "Relocation #" << std::dec << i << " of " << section.name()
                       << " symbol index is out-of-bound";
        }
      }
...
template <class MACHO_T> void BinaryParser::parse_dyldinfo_rebases() {
  ...
  while (not done and this->stream_->pos() < end_offset) {
    ...
    case REBASE_OPCODES::REBASE_OPCODE_ADD_ADDR_ULEB: {
      ...
      if (current_segmment == nullptr or segment_offset > current_segmment->file_size()) {
        LOG(WARNING) << "REBASE_OPCODE_ADD_ADDR_ULEB: Bad offset (" << std::hex << std::showbase << segment_offset
                     << " > " << current_segmment->file_size() << ")";
      }
    ...
    case REBASE_OPCODES::REBASE_OPCODE_DO_REBASE_ULEB_TIMES: {
      ...
      for (size_t i = 0; i < count; ++i) {
        ...
        if (current_segmment == nullptr or segment_offset > current_segmment->file_size()) {
          LOG(WARNING) << "REBASE_OPCODE_DO_REBASE_ADD_ADDR_ULEB: Bad offset (" << std::hex << std::showbase
...
template <class MACHO_T> void BinaryParser::parse_dyldinfo_binds() {
...
// Generic bindings
// ================
template <class MACHO_T> void BinaryParser::parse_dyldinfo_generic_bind() {
  using pint_t = typename MACHO_T::uint;
  ...
  while (not done and this->stream_->pos() < end_offset) {
    ...
    case BIND_OPCODES::BIND_OPCODE_DO_BIND: {
      this->do_bind<MACHO_T>(BINDING_CLASS::BIND_CLASS_STANDARD, type, segment_idx, segment_offset, symbol_name,
                             library_ordinal, addend, is_weak_import, false, segments);
      segment_offset += sizeof(pint_t);
      break;
    }
...
// Weak binding
// ============
template <class MACHO_T> void BinaryParser::parse_dyldinfo_weak_bind() {
  using pint_t = typename MACHO_T::uint;
  ...
  while (not done and this->stream_->pos() < end_offset) {
    ...
    case BIND_OPCODES::BIND_OPCODE_DO_BIND: {
      this->do_bind<MACHO_T>(BINDING_CLASS::BIND_CLASS_WEAK, type, segment_idx, segment_offset, symbol_name, 0, addend,
    ...
    case BIND_OPCODES::BIND_OPCODE_DO_BIND_ULEB_TIMES_SKIPPING_ULEB: {
      ...
      for (size_t i = 0; i < count; ++i) {
        this->do_bind<MACHO_T>(BINDING_CLASS::BIND_CLASS_WEAK, type, segment_idx, segment_offset, symbol_name, 0,
                               addend, is_weak_import, is_non_weak_definition, segments);
        segment_offset += skip + sizeof(pint_t);
      }
      break;
    }
...
// Lazy binding
// ============
template <class MACHO_T> void BinaryParser::parse_dyldinfo_lazy_bind() {
  using pint_t = typename MACHO_T::uint;
  ...
  while (this->stream_->pos() < end_offset) {
    ...
    case BIND_OPCODES::BIND_OPCODE_DO_BIND: {
      this->do_bind<MACHO_T>(BINDING_CLASS::BIND_CLASS_LAZY, static_cast<uint8_t>(BIND_TYPES::BIND_TYPE_POINTER),
                             segment_idx, segment_offset, symbol_name, library_ordinal, addend, is_weak_import, false,
                             segments);
      segment_offset += sizeof(pint_t);
      break;
    }
...
void BinaryParser::do_bind(BINDING_CLASS cls, uint8_t type, uint8_t segment_idx, uint64_t segment_offset,
                           const std::string &symbol_name, int32_t ord, int64_t addend, bool is_weak,
                           bool is_non_weak_definition, it_segments &segments) {
  ...
  std::unique_ptr<BindingInfo> binding_info{
      new BindingInfo{cls, static_cast<BIND_TYPES>(type), address, addend, ord, is_weak, is_non_weak_definition}};
...
template <class MACHO_T> void BinaryParser::do_rebase(uint8_t type, uint8_t segment_idx, uint64_t segment_offset) {
  using pint_t = typename MACHO_T::uint;
  ...
  // Tie symbol
  auto &&it_symbol = std::find_if(std::begin(this->binary_->symbols_), std::end(this->binary_->symbols_),
                                  [address](const Symbol *sym) { return sym->value() == address; });
  ...
  default: { LOG(ERROR) << "Unsuported relocation type: 0x" << std::hex << type; }
  }
}
